...
const marketCategories = ["全部算法", "目标检测", "语义分割", "异常分析", "多模态识别"];

function renderMarketTabs() {
  categoryTabsEl.innerHTML = marketCategories.map((item) => `
    <button class="tab-pill ${marketState.selectedCategory === item ? "active" : ""}" data-category="${item}">${item}</button>
  `).join("");

  industryTabsEl.innerHTML = ["全部", ...platformData.industries.map((item) => item.name)].map((item) => `
    <button class="tab-pill ${marketState.selectedIndustry === item ? "active" : ""}" data-industry="${item}">${item}</button>
  `).join("");
...
function renderDetail() {
  ...
  if (!item) {
    detailPanelEl.innerHTML = "<div class=\"empty-detail\"><p class=\"eyebrow\">Algorithm Detail</p><h3>选择一个算法查看详情</h3><p>这里会展示算法简介、技术栈和场景适配信息。</p></div>";
    return;
  }
  ...
  detailPanelEl.innerHTML = `
    ...
    <div class="detail-actions">
      <button class="primary-btn">申请演示</button>
      <button class="ghost-btn">获取资料</button>
    </div>
  `;
}
